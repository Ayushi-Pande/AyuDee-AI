    ...
    <div className={`${largeText ? "large-text" : ""} ${highContrast ? "high-contrast" : ""} app-page text-slate-800`}>
      <div className="flex min-h-screen gap-0">
        <aside className="hidden w-80 shrink-0 bg-[linear-gradient(155deg,#041F33_0%,#062A45_60%,#087EA4_150%)] p-7 text-white shadow-[12px_0_35px_rgba(4,31,51,0.16)] lg:flex lg:flex-col">
          <div className="mb-8"><AyuDeeLogo variant="light" /></div>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 rounded-[18px] px-4 py-3 text-base font-medium transition ${
                    isActive
                      ? "bg-[#14B8A6] text-[#041F33] shadow-[0_8px_20px_rgba(20,184,166,0.22)]"
        ...
        <div className="mx-auto grid max-w-md grid-cols-4 gap-2">
          {navItems.map(({ to, label, icon: Icon }) => (
            <NavLink
              ...
              to={to}
              className={({ isActive }) =>
                `flex flex-col items-center justify-center gap-1 rounded-2xl px-2 py-2 text-[9px] font-semibold ${
